---
title: Configuração
description: Todos os campos do livetest.config.json, com padrões, exemplos e o motivo de cada um.
---

# Configuração

O Live Test Runner procura, subindo a partir do diretório atual, o primeiro arquivo com um
destes nomes:
...
O diretório do arquivo encontrado vira a **raiz do projeto**, e todos os globs e caminhos
relativos são resolvidos a partir dela. Sem nenhum arquivo, os valores padrão são usados e
a raiz é o diretório atual.

Arquivos `.json` aceitam comentários `//` e `/* */`, que são removidos antes do parse.
Módulos `.js`, `.mjs` e `.cjs` devem exportar a configuração como `default`.
...
`string[]`. Globs **somados** aos ignores padrão (`node_modules`, `dist`, `build`,
`coverage`, `__pycache__`, `.venv`, `.livetest`, entre outros) e ao `.gitignore`. Aplicados
também a diretórios, o que poda a varredura antes de descer neles.

## useGitignore

`boolean`. Lê o `.gitignore` da raiz e converte suas regras em globs de exclusão. Negações
(`!padrão`) **não** são suportadas e geram aviso.
...
Há um tratamento mais longo, com demonstração interativa, em
...
Um objeto por linguagem ou framework. A chave é livre (`"js"`, `"python"`, `"go"`) e
...
::: info Declarar runners restringe
Declarar `runners` **restringe** o daemon aos runners declarados. Para `"js"` e `"python"`,
o que você escrever é mesclado sobre a base embutida, então `{"js": {"adapter": "jest"}}`
mantém `match`, `testPatterns` e o resto.
:::
...
Templates que **não** começam com `{dir}` são resolvidos a partir da raiz, o que cobre o
layout `tests/` no topo do repositório. Barras duplicadas por tokens vazios são colapsadas.
...
Porta de entrada para qualquer linguagem sem adapter dedicado. O comando recebe os arquivos
de teste como argumentos finais e o resultado vem do **código de saída**, sem casos
individuais.
...
| Campo | Tipo | Padrão | Significado |
|---|---|---|---|
| `logFile` | `string` ou `null` | `".livetest/run.log"` | log NDJSON append-only; rotaciona em 5 MB |
| `statusFile` | `string` ou `null` | `".livetest/status.json"` | snapshot do estado, escrito atomicamente |
| `stdout` | `boolean` | `true` | emitir relatório no stdout |
| `format` | `"pretty"` ou `"ndjson"` | `"pretty"` | formato do stdout |
| `color` | `boolean` ou `"auto"` | `"auto"` | `"auto"` respeita TTY, `NO_COLOR` e `FORCE_COLOR` |
| `logTailLines` | `number` | `40` | linhas de stdout e stderr guardadas por execução |
...
| `port` | `number` | `0` | `0` deixa o SO escolher, o que evita colisão entre projetos |
| `discoveryFile` | `string` | `".livetest/daemon.json"` | onde o daemon publica pid, host e porta |

Se a porta não puder ser aberta, o daemon **não aborta**: emite um aviso e segue com stdout
e log de arquivo. Veja [Protocolo de eventos](/reference/protocol).
...
A lista completa está na [referência da CLI](/reference/cli).
...
Campos desconhecidos geram aviso, não erro, então configurações de versões futuras não
quebram versões antigas. O `livetest doctor` roda a mesma validação e ainda verifica se os
binários dos runners existem.
