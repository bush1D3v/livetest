---
title: API do core
description: A superfície programática do @livetest/core: engine, configuração, grafo, planner e o ponto de entrada de cliente.
---

# API do core

O `@livetest/core` é o motor: watcher, debounce, grafo de dependências, execução
incremental de testes e os três canais de saída. É agnóstico de linguagem e independente de
CLI e de editor.
...
O índice principal arrasta o compilador TypeScript, usado pelo grafo JS/TS. Um consumidor
que apenas lê eventos, como uma extensão de editor ou um dashboard, deve importar de
`/client`: no bundle da extensão do VSCode isso é a diferença entre 9 MB e 24 KB.
...
como separador**, inclusive no Windows. A conversão para o formato nativo acontece só na
...
Veja [Escrevendo um adapter](/guide/adapters).

## loadConfig

`loadConfig(options): Promise<LoadConfigResult>`

Procura o arquivo subindo a partir de `cwd`, valida, mescla os padrões e normaliza. Lança
`LiveTestError` com código `CONFIG_INVALID` listando **todos** os problemas encontrados,
...
O `chain` é o caminho completo de importação. É dele que sai a frase *"rodou porque
header.ts importa login.ts"*.
...
O protocolo está documentado em [Protocolo de eventos](/reference/protocol).
...
::: info Falhas de runtime não lançam
Adapter que quebra, Python ausente, disco cheio: tudo vira um evento `error` com
`degradedTo`, e o daemon continua. Robustez, aqui, significa degradar e não sumir.
:::
