---
title: CLI
description: Todos os comandos do livetest, flags globais, sobrescritas de configuração e códigos de saída.
---
...
Sobe o daemon em primeiro plano: observa o projeto, roda os testes afetados a cada save,
escreve `run.log` e `status.json` e abre o canal de eventos para a extensão do VSCode.
Encerre com `Ctrl+C`; o arquivo de descoberta é removido na saída.
...
Explica o que rodaria e por quê, **sem executar nada**. O comando para quando a seleção de
testes surpreende.
...
Também mostra os imports que o grafo não conseguiu resolver, que é o aviso de que a
...
Estado do daemon e resultado do último lote. Tenta primeiro o canal de eventos, que é dado
sempre atual, e cai para `.livetest/status.json`, que sobrevive ao encerramento.

## livetest watch

Acompanha, em um segundo terminal, um daemon já em execução. Quando o daemon foi iniciado
em background por um agente, é assim que uma pessoa vê o mesmo fluxo.
...
limpeza precisa acontecer aqui. Também detecta e remove registro órfão de um daemon que
morreu sem limpar. O `--timeout` ajusta a espera (padrão 4000 ms).
...
A CLI é embutível, o que é útil em um script de build ou plugin de editor, sem criar
processo:
...
O `runCli` **não chama `process.exit`**: devolve o código e deixa a decisão com quem
